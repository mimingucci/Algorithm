    ...
    public static void main(String[] args) {
        ...
        while((t--)>0){
           ...
           for(int i=0; i<s.length(); i++){
               a[i+1]=(int)s.charAt(i)-96;
           }
           int n=s.length();
               Vector<Integer>[] ans=new Vector[27];
               for(int i=0; i<27; i++)ans[i]=new Vector<Integer>();
               int bns=0;
               for(int i=1; i<=n; i++){
                   if((a[i]>=Math.min(a[1], a[n]) && a[i]<=Math.max(a[n], a[1])) ){
                       ans[a[i]].add(i);
                       ++bns;
                   }
               }
               pw.println(Math.abs(a[1]-a[n])+" "+bns);
               if(a[1]==a[n]){
                   for(int j : ans[a[1]]){
                       pw.print(j+" ");
                   }
